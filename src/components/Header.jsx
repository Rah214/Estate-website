...
const navLinks = [
    ...
    {
        href: "/resources", label: "Resources", submenu: [
            { href: "/property", label: "Manage Property", submenu: [] },
...
const authButtons = [
    { href: "/login", label: "Login", style: "border-2 border-[#E0DEF7] text-[#000929]" },
            ...
            <div className="flex justify-between items-center">
                ...
                <nav className="hidden md:flex lg:gap-12 gap-6 px-3">
                    {navLinks.map(({ href, label, submenu }) => (
                        <Link key={href} href={href} className="md:leading-6 font-normal text-[#000929] flex items-center lg:text-[16px] text-[14px] md:gap-2 hover:text-[#7065F0] transition-colors duration-200">
                            {label}
                            {submenu && (
                                <svg
                ...
                <div className="hidden md:flex items-center lg:gap-4 gap-2">
                    {authButtons.map(({ href, label, style }) => (
                        <Link key={href} href={href}>
                            <button className={`lg:px-6 lg:py-3 px-4 py-2 lg:text-[16px] text-[14px] lg:leading-6 leading-3 font-bold rounded-lg ${style} hover:opacity-80 transition-opacity duration-200`}>
                                {label}
                            </button>
                        </Link>
                    ))}
